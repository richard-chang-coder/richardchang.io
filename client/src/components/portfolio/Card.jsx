...
const CardContainer = styled(Paper)(({theme}) => ({
	...
	":hover": {
		...
		".card-button" : {
			...
			'-webkit-animation': `${MoveUpDown} 0.45s forwards ease-in-out`,
...
const Card = (portfolio) => {
	...
	return (
		<CardContainer 
			elevation={0} 
			sx={{bgcolor: portfolio.thumbnail.background_color, height: '100%'}} 
			onClick={() => navigate(`/portfolio/${portfolio._id}`, { state: portfolio})}
		...
		>
			<CardHeading 
				...
				variants={textMotion}
			>
				<CardSubtitle >{portfolio.start_date}{portfolio.end_date !== null ? '-' : null}{portfolio.end_date}</CardSubtitle>
				<CardTitle variant="h4" >{portfolio.thumbnail.title}</CardTitle>
				<CardLink variant="subtitle2"  className="card-button">View Case Study ></CardLink>
			</CardHeading>
			...
			<CardImage>
				...
				<motion.img  
					...
					src={process.env.PUBLIC_URL + '/mock-up.jpg'} alt="Dynamic Funds"
				/>
			</CardImage>
		</CardContainer>
	)
